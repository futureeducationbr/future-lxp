  ...
  <div
    ...
    <div
      ...
      <button
        ...
        <span className="text-xs hidden lg:inline-block mt-px"> {str("fechar")} </span>
      </button>
      <div>
        ...
        <div className="text-left mt-1 text-xs text-gray-800">
          ...
          | Some(teamName) =>
            <span>
              {"Enviado pelo time: " |> str}
              <span className="font-semibold"> {teamName |> str} </span>
              {" - " |> str}
            </span>
          | None => <span> {"Enviado por " |> str} </span>
          }}
          {
            let studentCount = submissionDetails |> SubmissionDetails.students |> Array.length

            submissionDetails |> SubmissionDetails.students |> Array.mapi((index, student) => {
